...
class Family {
  static final Map<String, Family> _families = {};
  ...
  Family._(this._all, this._one, this._exclude) : _index = _familyIndex++;
  ...
  bool matches(Entity entity) {
    ...
    if (_one.isNotEmpty && !_one.intersects(entityComponentBits)) {
      return false;
    }

    if (_exclude.isNotEmpty && _exclude.intersects(entityComponentBits)) {
      return false;
    }
  ...
  static String _getFamilyHash(Bits all, Bits one, Bits exclude) {
    ...
    if (all.isNotEmpty) {
      stringBuilder
        ..write("{all:")
        ..write(_getBitsString(all))
        ..write("}");
    }
    if (one.isNotEmpty) {
      stringBuilder
        ..write("{one:")
        ..write(_getBitsString(one))
        ..write("}");
    }
